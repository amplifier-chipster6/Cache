...
class CacheProviderWrapper(Provider):  # type: ignore[misc]
    ...
    async def complete(self, request, **kwargs):
            ...
            if embedding is not None:
                ...
                if results:
                    ...
                    if similarity >= threshold:
                        ...
                        # Below threshold: treat as skip, fall through to provider
                        self.semantic_skips += 1
                        semantic_skip = True
                else:
                    # No semantic results: miss, fall through to provider
        ...
        result = await self.provider.complete(request, **kwargs)
        self._last_metrics = self._build_metrics(
            exact_hit=False,
            semantic_hit=False,
